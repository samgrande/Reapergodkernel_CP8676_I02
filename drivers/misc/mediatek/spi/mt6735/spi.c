...
void __iomem *spi_cksys_base;
void __iomem *spi_pericfg_base ;

void spi_dump_sysclk_reg(void )
{
	u32 reg_val ,reg_val2;	
	reg_val =  __raw_readl(spi_cksys_base + 0x0060 );
	reg_val2 =  __raw_readl(spi_pericfg_base + 0x0018 ); 
	
	pr_warn("----debug--SPI cksys   reg = %x -------\n" , reg_val);
	pr_warn("----debug--SPI pericfg reg = %x -------\n" , reg_val2);
}
...
static int __init mt_spi_probe(struct platform_device *pdev)
{
	...
	unsigned int i;
#endif
	struct device_node * cksys_node ,* pericfg_node;

	cksys_node = of_find_compatible_node(NULL, NULL, "mediatek,CKSYS");
	spi_cksys_base = of_iomap(cksys_node, 0);
	
	pericfg_node = of_find_compatible_node(NULL, NULL, "mediatek,PERICFG");
	spi_pericfg_base = of_iomap(pericfg_node, 0);
