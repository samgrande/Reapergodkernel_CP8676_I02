...
static int mtkfb_probe(struct device *dev)
{
	...
	int                    r = 0;
	char *p = NULL;
    unsigned char lcd_info[50]={0}; //add for display lcd info in factory mode by xuzhixian@yulong.com 20150325
  ...
  primary_display_set_frame_buffer_address(fbdev->fb_va_base,fb_pa);
    ...
    {
        sprintf(lcd_info,"LCD: %s\n",mtkfb_lcm_name);
        get_device_info(lcd_info);
    }
    else
    {
        sprintf(lcd_info,"LCD: unknown LCD driver\n");
        get_device_info(lcd_info);
    }
    /*** add for display lcd info in factory mode by xuzhixian@yulong.com 20150325 end ***/
