...
//added for some sdcard cannot be detected  by zhoumaiyun 2015.08.10
static void msdc_recovery_sd_tune_path(struct msdc_host *host)
{
    void __iomem *base = host->base;
    printk(" \nmsdc_recovery_sd_tune_path enter \n");
...
#endif
    printk(" \nmsdc_recovery_sd_tune_path leave \n");
}
//added for some sdcard cannot be detected  by zhoumaiyun 2015.08.10
...
#endif
			msdc_recovery_sd_tune_path(host); //added for some sdcard cannot be detected  by zhoumaiyun 2015.08.10
			...
			default:	/* Response types 1, 3, 4, 5, 6, 7(1b) */
				*rsp = sdr_read32(SDC_RESP0);
				...
				if (((cmd->opcode == 13) || (cmd->opcode == 25)) && (*rsp & R1_OUT_OF_RANGE) 
					&& (host->hw->host_function == MSDC_EMMC)) { //added MTK Patch P77 by zhoumaiyun 2015.08.05
					pr_err("[%s]: msdc%d XXX CMD<%d> resp<0x%.8x>,bit31=1,force make crc error\n",
						__func__, host->id, cmd->opcode, *rsp);
					cmd->error = (unsigned int)-EIO;
					//added MTK Patch P77 by zhoumaiyun 2015.08.05
					if (cmd->opcode == 25)
						msdc_reset_hw(host->id);
				    //added MTK Patch P77 by zhoumaiyun 2015.08.05
				}
