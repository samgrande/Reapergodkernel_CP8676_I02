...
void rdma_set_ultra(unsigned int idx, unsigned int width, unsigned int height, unsigned int bpp, unsigned int frame_rate,void * handle)
{
  ...
  if (idx == 0)
  {
      ...
      else if (width > 540)
      {
          ...
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 0x000000A8);//168//2015.08.07 lijianbin@yulong.com add for MTK patch 80
      }
      else if (width > 480)
      {
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x32010D1C);
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 0x000000A3);//163//2015.08.07 lijianbin@yulong.com add for MTK patch 80
      }
      else
      {
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x39010A16);
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 0x000000A5);//165//2015.08.07 lijianbin@yulong.com add for MTK patch 80
      }
...
DDP_MODULE_DRIVER ddp_driver_rdma =
{
    ...
    .build_cmdq      = NULL,
    .set_lcm_utils   = NULL,
    .enable_irq    = rdma_enable_irq//2015.08.07 lijianbin@yulong.com add for MTK patch 80
}; 
