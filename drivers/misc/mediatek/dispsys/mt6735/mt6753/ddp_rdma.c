...
void rdma_set_ultra(unsigned int idx, unsigned int width, unsigned int height, unsigned int bpp, unsigned int frame_rate,void * handle, enum RDMA_MODE mode, bool isidle)
{
  ...
  if (idx == 0)
  {
      ...
      DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 0xFF);//0xff//2015.08.07 lijianbin@yulong.com add for MTK patch 80
      ...
      if (width > 800)// FHD
      {
		  ...
		  if (mode == RDMA_MODE_DIRECT_LINK)
		  {
		     sodi_threshold = 168|(475<<10);//low:168	high:475 (direct link in FHD)	
              fifo_valid_size = 0x70+1;//2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016
          }
		  else
          {
             if (isidle)
			 {
                sodi_threshold = 84|(158<<10);//low:84 high:158 (decouple in FHD in idle)//2015.08.07 lijianbin@yulong.com add for MTK patch 80
				DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0,0x1c01371c);
                 fifo_valid_size = 0x20;//2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016
             }
			 else
			 {
      ...
      else // HD
      {
		  ...
		  if (mode == RDMA_MODE_DIRECT_LINK)
		  {
		     sodi_threshold = 75|(328<<10);//low:75	high:328 (direct link in HD)
              fifo_valid_size = 0x32+1;//2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016
          }
          else
          {
			  if (isidle)
...
//      DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, gRDMAFIFOLen);//2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016
    }
    else
    {
	    DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_FIFO_UNDERFLOW_EN, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, 1);	    
