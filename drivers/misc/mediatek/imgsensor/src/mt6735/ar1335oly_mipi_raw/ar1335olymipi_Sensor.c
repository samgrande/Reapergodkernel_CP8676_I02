...
#include "ar1335olymipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "ar1335oly_camera_sensor"
#define LOG_1 LOG_INF("AR1335OLY,MIPI 4LANE\n")
...
/****************************   Modify end    *******************************************/
#define AR1335OLY_DEBUG

#ifdef AR1335OLY_DEBUG
...
#define AR1335OLY_OTP

extern kal_uint16 OTPData[];
...
#if defined(AR1335OLY_OTP)
static kal_uint16 awb_R_Gr_ratio=1024;
static kal_uint16 awb_B_Gr_ratio= 1024;
static kal_uint16 awb_Gb_Gr_ratio= 1024;

static int RG_Ratio_Typical = 0x23f;
static int BG_Ratio_Typical = 0x294;
static int GG_Ratio_Typical = 0x400;


static void AR1335OLY_otp_read(kal_uint16 otp_type){   // here type 1100 或者是其他type 3700
	...
	do{
		...
		if((flag_end&0x0060) == 0x60){
			LOG_INF("AR1335OLY read otpm successful 0x304A = 0x%x  ,otp_type =0x%x \n",flag_end,otp_type);
			break;
		}
		mDELAY(10);
		i++;
		LOG_INF("AR1335OLY read otpm error 0x304A = 0x%x,i=%d  ,otp_type =0x%x \n",flag_end,i,otp_type);
	}while(i<3);
}
static void AR1335OLY_OTP_AUTO_LOAD_LSC(void)
{
	kal_uint16 temp;
	AR1335OLY_otp_read(0x11);
    LOG_INF("AR1335OLY updata AR1335OLY_OTP_AUTO_LOAD_LSC !\n");
	// enable sc
	temp = read_cmos_sensor_2(0x3780);  //
	write_cmos_sensor_2byte(0x3780,temp|0x8000);
}
static void AR1335OLY_otp_read_awb()
{
	kal_uint16 temp,temp1;
		...
		kal_uint16 Checksum;
	AR1335OLY_otp_read(0x30);
	temp = read_cmos_sensor_2(0x3800); /// check awb whether exist bit[3]AWB DATA EXIST bit: 0->no exit AWB data; 1->awb data exist
	LOG_INF("AR1335OLY read otpm AWB enter 30-3800 temp=0x %x !\n",temp);
	temp = temp&0x0008;
	LOG_INF("AR1335OLY read otpm AWB enter 1 30-3800 temp=0x %x !\n",temp);
	if(temp==0x0008)
		{
			LOG_INF("AR1335OLY read otpm AWB exist !!!!!\n");

				AR1335OLY_otp_read(0x37); /// read AWB data;
				temp = read_cmos_sensor_2(0x3806);  // read the type37 flag

				LOG_INF("AR1335OLY read otpm AWB enter 37- 3806 temp=0x %x !\n",temp);

				temp1 = read_cmos_sensor_2(0x304A);
				LOG_INF("AR1335OLY read otpm AWB enter 37- 304a temp1=0x %x !\n",temp1);
				temp1 = temp1&0x0040;
				LOG_INF("AR1335OLY read otpm AWB enter 5 30-304a temp1=0x %x !\n",temp1);
				if((temp ==0xFFFF)&&(temp1==0x0040))  // if the flag == 0xffff&0x304A[6]==1 then read AWB
				{
						...
						OTPData[0] = awb_R_Gr_ratio;//
						OTPData[1] = awb_B_Gr_ratio; //
						OTPData[2] = awb_Gb_Gr_ratio; //
						OTPData[3] = RG_Ratio_Typical;
						OTPData[4] = BG_Ratio_Typical;
						OTPData[5] = GG_Ratio_Typical;
						...
						LOG_INF("AR1335OLY read otpm AWB enter Checksum1 =0x %x !\n",Checksum1);
						AR1335OLY_otp_read(0x38); /// read awb checksum data;
						Checksum = read_cmos_sensor_2(0x3800);
						LOG_INF("AR1335OLY read otpm AWB enter Checksum =0x %x !\n",Checksum);
	...
	LOG_INF("AR1335OLY read otpm AWB exit !\n");
...
//// end init
	...
	//LOG_INF("AR1335OLY read auto load lsc reg[0x%x] = 0x%x\n",0x3812,temp);
//
//        LOG_INF("sensor_init Enter otp read LSC START \n");
//		AR1335OLY_OTP_AUTO_LOAD_LSC();
//		LOG_INF("sensor_init Enter otp read LSC EXIT\n");
//		 LOG_INF("sensor_init Enter otp read AWB START \n");
//		AR1335OLY_otp_read_awb();
//		LOG_INF("sensor_init Enter otp read AWB EXIT \n");
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    {
        LOG_INF("Read sensor_moudle_id ==  oly ar1335\n");
        *sensor_id = AR1335_SENSOR_ID;
        sprintf(factory_module_id,"main camera: ar1335 oly \n");
    }
    else
    {
        *sensor_id = 0xffffffff;
    ...
    //AR1335OLY_OTP_AUTO_LOAD_LSC();
	//AR1335OLY_otp_read_awb(); //gaoatao modify  error 2015-4-4 20:33:06
    info_limit++;
    return ERROR_NONE;
}
...
UINT32 AR1335OLY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
