...
static void _i2c_write_reg(mt_i2c *i2c)
{
  ...
  /*set start condition */
  if(i2c->speed <= 100){
    i2c_writel(i2c,OFFSET_EXT_CONF, 0xd001);//modify for start and stop conndition timing by qinxinjun@yulong.com in 20150611
  } else {
...
static S32 mt_i2c_probe(struct platform_device *pdev)
{
  ...
  if (of_property_read_u32(pdev->dev.of_node, "def_speed", &pdev->id)) {
	...
	i2c->defaul_speed  = 100;
  	...
  	i2c->defaul_speed = pdev->id;	//modify by zhanyd for default_speed. 2015-5-20
  }
#else
