...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	...
	switch(pmic_type){
		case MT65XX_LED_PMIC_NLED_ISINK0:
            ...
            pmic_set_register_value(PMIC_ISINK_CH0_STEP,ISINK_0);//modify for reduce led light by sunxiaogang@yulong.com 20150602
		...
		case MT65XX_LED_PMIC_NLED_ISINK1:
            ...
            pmic_set_register_value(PMIC_ISINK_CH1_STEP,ISINK_0);//modify for reduce led light by sunxiaogang@yulong.com 20150602
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	mutex_lock(&leds_pmic_mutex);
		if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK0)
		{
			...
			pmic_set_register_value(PMIC_RG_DRV_ISINK0_CK_CKSEL,0);
			pmic_set_register_value(PMIC_ISINK_CH0_MODE,PMIC_PWM_0);
            pmic_set_register_value(PMIC_ISINK_CH0_STEP,ISINK_1);//modify for reduce led light by xuzhixian@yulong.com 20150508
			pmic_set_register_value(PMIC_ISINK_DIM0_DUTY,15);
		...
		else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK1)
		{
			...
			pmic_set_register_value(PMIC_RG_DRV_ISINK1_CK_CKSEL,0);
			pmic_set_register_value(PMIC_ISINK_CH1_MODE,PMIC_PWM_0);
            pmic_set_register_value(PMIC_ISINK_CH1_STEP,ISINK_1);//modify for reduce led light by xuzhixian@yulong.com 20150508
			pmic_set_register_value(PMIC_ISINK_DIM1_DUTY,15);
...
// add for reduce power dissipation by xuzhixian@yulong.com 20150506 start
...
// add for reduce power dissipation by xuzhixian@yulong.com 20150506 end
...
void mt_mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	if(led_data->level != level)
	{
		...
		else
		{
            ...
            //add for reduce power dissipation by xuzhixian@yulong.com 20150507 start
            level = (unsigned char)transform[255-led_data->level];
            //add for reduce power dissipation by xuzhixian@yulong.com 20150507 end
			//mt_mt65xx_led_set_cust(&led_data->cust, led_data->level);	
