...
//extern int PMIC_IMM_GetOneChannelValue(upmu_adc_chl_list_enum dwChannel, int deCount, int trimd);  //get battery id using pmic adc
...
char doddbg[100];
/* debug by zhanyongding@yulong.com 2015-6-25 start */
char doddbg1[100];
char doddbg2[100];
/* debug by zhanyongding@yulong.com 2015-6-25 end */
...
void dod_init(void)
{
    ...
    /* debug by zhanyongding@yulong.com 2015-6-25 start */
    sprintf(doddbg1,"hwocv:(%d:%d) swocv:(%d,%d), %d %d %d\n",gFG_voltage,gFG_capacity_by_v,gFG_voltage_init,gFG_capacity_by_v_init,
	...
	CUST_POWERON_DELTA_CAPACITY_TOLRANCE,CUST_POWERON_LOW_CAPACITY_TOLRANCE,CUST_POWERON_DELTA_HW_SW_OCV_CAPACITY_TOLRANCE);
        ...
        bm_print(BM_LOG_CRTI, "[FGADC]is_battery_remove enable\n");
...
#else
bm_print(BM_LOG_CRTI, "[FGADC]is_battery_remove disable\n");
#if defined(SOC_BY_HW_FG)
#if defined(INIT_SOC_BY_SW_SOC)
	if (((g_rtc_fg_soc != 0)
	     && (((abs(g_rtc_fg_soc - gFG_capacity_by_v)) <= CUST_POWERON_DELTA_CAPACITY_TOLRANCE)
    ...
    /* debug by zhanyongding@yulong.com 2015-6-25 start */
    sprintf(doddbg2, "rtc:%d, g_boot_reason:%d, g_boot_mode:%d, gFG_DOD0:%d, g_tracking_point:%d, gFG_15_vlot:%d\n",
       g_rtc_fg_soc, g_boot_reason, g_boot_mode, gFG_DOD0, g_tracking_point, gFG_15_vlot);
    /* debug by zhanyongding@yulong.com 2015-6-25 end */
...
/* 1. Get Raw Data */
    ...
    /* debug by zhanyongding@yulong.com 2015-6-25 start */
    sprintf(doddbg,"[FGADC] SWOCV : %d,%d,%d,%d,%d,%d\n",
        gFG_voltage_init, gFG_voltage, gFG_current, gFG_Is_Charging, gFG_resistance_bat,
        gFG_compensate_value);
    /* debug by zhanyongding@yulong.com 2015-6-25 end */
...
/* 4. update DOD0 */
    ...
    bm_print(BM_LOG_CRTI, "[FGADC]rfg detect enable\n");
...
#endif
    bm_print(BM_LOG_CRTI, "[FGADC]rfg detect disable\n");
/* 5. Logging */
	bm_print(BM_LOG_CRTI,
		 "[FGADC] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",
		 gFG_Is_Charging, gFG_current, gFG_columb, gFG_voltage, gFG_capacity_by_v,
