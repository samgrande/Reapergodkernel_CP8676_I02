...
static int __devinit fpc1020_probe(struct spi_device *spi)
{
    ...
    mt_set_gpio_mode(GPIO_FPC_SPI_M0SI, GPIO_MODE_01);
	...
	fpc1020_pdata->cs_gpio    = GPIO_FPC_SPI_CS;
    ...
    if (error)
	{
		printk("fpc1020_worker_init--error=%d\n",error);	
	}

	error = fpc1020_calc_finger_detect_threshold_min(fpc1020);
    if (error)
	{
		printk("fpc1020_calc_finger_detect_threshold_min=%d\n",error);	
	}	
    ...
    if (error)
	{
		printk("fpc1020_set_finger_detect_threshold=%d\n",error);	
	}	
					...
					struct fpc1020_platform_data *pdata)
{
	...
	// set irq gpio
  	error = mt_set_gpio_mode(fpc1020->irq_gpio, GPIO_MODE_01);// set EINT MODE by luck.wang@yulong.com 2015-03-31
	if (error != 0) {
...
{
    printk(KERN_INFO "%s\n", __func__);
    spi_register_board_info(fpc1020_board_devs, ARRAY_SIZE(fpc1020_board_devs)); //add by lucky.wang@yulong.com 2015-03-31
