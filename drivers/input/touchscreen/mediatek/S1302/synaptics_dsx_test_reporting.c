...
DECLARE_COMPLETION(f54_remove_complete_alt);
...
exit:
	complete(&f54_remove_complete_alt);
...
{
	synaptics_rmi4_new_function_alt(&f54_module, true);
...
{
	synaptics_rmi4_new_function_alt(&f54_module, false);

	wait_for_completion(&f54_remove_complete_alt);
