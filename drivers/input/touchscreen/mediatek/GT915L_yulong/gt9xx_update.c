...
#if ( GTP_HEADER_FW_UPDATE || GTP_COMPATIBLE_MODE ) //modify for TW firmware update fail by qinxinjun@yulong.com in 20150708
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
            ...
            (!memcmp(fw_head->pid, "91XX", 4)))
    {
        ...
        //The third condition 
        if (fw_head->vid != update_msg.ic_fw_msg.vid) //modify for fw upgrading by qinxinjun@yulong.com in 20150516
        {
...
#if (GTP_HEADER_FW_UPDATE || GTP_AUTO_UPDATE_CFG) //modify for TW firmware update fail by qinxinjun@yulong.com in 20150708
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head *fw_head, u8 *path)
{
    ...
    {
#if 1 //modify for TW firmware update fail by qinxinjun@yulong.com in 20150708
...
{
#if (GTP_HEADER_FW_UPDATE) //modify for TW firmware update fail by qinxinjun@yulong.com in 20150708
...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
    ...
    {
        GTP_ERROR("Inconsistent firmware size. File size: %d, default fw size: %d", update_msg.fw_total_len, (int)sizeof(gtp_default_FW_fl));
        set_fs(update_msg.old_fs);
        _CLOSE_FILE(update_msg.file);
        return FAIL;
    }
