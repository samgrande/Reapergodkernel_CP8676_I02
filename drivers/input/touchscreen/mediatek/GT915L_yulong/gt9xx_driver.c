...
#include <mach/mt_boot.h> //open TW auto update fw by qinxinjun@yulong.com in 20150715
...
//#define TPD_SMART_HULL   //add by sunxuebin for smart hull
#ifdef TPD_SMART_HULL
static u8 pre_tpd_up_flag = 0;    
...
//add by sunxuebin start for gesture wake up
...
char wakeup_string_buffer[64];//modify for gesture wakeup show by qinxinjun@yulong in 20150516

extern struct device *touchscreen_get_dev(void);
//add by sunxuebin end for gesture wake up
#endif

//boen 0 laibao 1 oufei 2

#define TW_BOEN_ID     0
#define TW_SHENYUE_ID     1   
#define TW_OFILM_ID    2
//#define TW_TOPTOUCH_ID 3
//#define TW_JUNDA_ID    4
//#define TW_BAOMING_ID    5
...
touch_mode_type tp_mode; //add by sunxuebin 2013-11-17 for hand and glove mode 
...
/*start to window mode debug by qinxinjun@yulong.com in 20150516*/
...
/*end to window mode debug by qinxinjun@yulong.com in 20150516*/
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    u8 opr_buf[16];
    u8 sensor_id = 0;
    //delete to window mode debug by qinxinjun@yulong.com in 20150516
...
/*start to window mode debug for backup group config version by qinxinjun@yulong.com in 20150724*/
#if TPD_SMART_HULL
            if (opr_buf[0] < 90)
            {
...
#endif
/*end to window mode debug for backup group config version by qinxinjun@yulong.com in 20150724*/
        }
        else
        {
...
int goodix_firmware_need_update(void)
{
    ...
    {
        GTP_DEBUG("GT915L updates FW right now, please!!!!!\n");
           return 1;
    }       
    else
    {    
        GTP_DEBUG("GT915L doesn't need to update FW now!!!!!\n");
        return 0;
    } 
...
/*start to add get device info function for factory mode by qinxinjun@yulong.com 20150519 */
extern int get_device_info(char* buf);
/*end to add get device info function for factory mode by qinxinjun@yulong.com 20150519 */
...
int goodix_get_info(void)
{
    ...
    {
        sprintf(buf, "TW : BOEN:GT915L 0x%04x:%2x\n", version_info,cfg_info_group1[0]); //Display the right firmware version
        /*start to add get device info function for factory mode by qinxinjun@yulong.com 20150519 */
        get_device_info(buf);
        /*end to add get device info function for factory mode by qinxinjun@yulong.com 20150519 */
        return ret;
    }
    /*start qinxinjun@yulong.com 20141016 modify module ID from yeji to laibo*/
    if(tp_type == TW_SHENYUE_ID)  
    {
        sprintf(buf, "TW : SHENYUE:GT915L 0x%04x:%2x\n", version_info,cfg_info_group2[0]); //Display the right firmware version
        /*start to add get device info function for factory mode by qinxinjun@yulong.com 20150519 */
        get_device_info(buf);
        /*end to add get device info function for factory mode by qinxinjun@yulong.com 20150519 */
        return ret;
    }
    /*end qinxinjun@yulong.com 20141016 modify module ID from yeji to laibo*/
    if(tp_type == TW_OFILM_ID) 
    {
        sprintf(buf, "TW : OFILM:GT915L 0x%04x:%2x\n",version_info,cfg_info_group3[0]); //Display the right firmware version
        /*start to add get device info function for factory mode by qinxinjun@yulong.com 20150519 */
        get_device_info(buf);
        /*end to add get device info function for factory mode by qinxinjun@yulong.com 20150519 */
        return ret;
...
//    {
//        sprintf(buf, "TW : TOPTOUCH:GT915L 0x%04x\n",version_info); //Display the right firmware version
//        get_device_info(buf);
//        return ret;
//    }
//    if(tp_type == TW_JUNDA_ID) 
//    {
//        sprintf(buf, "TW : JUNDA:GT915L 0x%04x\n",version_info); //Display the right firmware version
//        //get_device_info(buf);
//        return ret;
//    }
    //if(tp_type == TW_BAOMING_ID) 
    //{
    //    sprintf(buf, "TW : BAOMING:GT915L 0x%04x:%2x\n",version_info,cfg_info_group6[0]); //Display the right firmware version
    //    /*start qinxinjun@yulong.com 20141022 add get device info function for factory mode*/
    //    //get_device_info(buf);
    //    /*end qinxinjun@yulong.com 20141022 add get device info function for factory mode*/
    //    return ret;
    //}
...
int goodix_get_firmware_version( char *buf)
{
    ...
    if(tp_type == TW_BOEN_ID) {
        return sprintf(buf, "%s%04x:%2x","BOEN:GT915L:0x",version_info,cfg_info_group1[0]);
    }
    /*start qinxinjun@yulong.com 20141016 modify module ID from yeji to laibo*/
    if(tp_type == TW_SHENYUE_ID) {
        return sprintf(buf, "%s%04x:%2x","SHENYUE:GT915L:0x",version_info,cfg_info_group2[0]);
    }
    /*end qinxinjun@yulong.com 20141016 modify module ID from yeji to laibo*/
    if(tp_type == TW_OFILM_ID) {
        return sprintf(buf, "%s%04x:%2x","OFILM:GT915L:0x",version_info,cfg_info_group3[0]);
    }
//    if(tp_type == TW_TOPTOUCH_ID) {
//        return sprintf(buf, "%s%04x:%2x","TOPTOUCH:GT915L:0x",version_info,cfg_info_group4[0]);
//    }
//    if(tp_type == TW_JUNDA_ID) {
//        return sprintf(buf, "%s%04x:%2x","JUNDA:GT915L:0x",version_info,cfg_info_group6[0]);
//    }
    //if(tp_type == TW_BAOMING_ID) {
    //    return sprintf(buf, "%s%04x:%2x","BAOMING:GT915L:0x",version_info,cfg_info_group6[0]);
    //}
...
ssize_t goodix_firmware_do_update(void)
{
    int ret = -1 ;
...
{
/*start to modify for gesture wakeup show by qinxinjun@yulong in 20150516*/
    char *buffer = wakeup_string_buffer;
    sprintf(buffer,"NONE");
    ...
    {
        sprintf(buffer,"double_click");
    }
    buffer = NULL;
/*end to modify for gesture wakeup show by qinxinjun@yulong in 20150516*/
...
#endif
/*start open TW auto update fw by qinxinjun@yulong.com in 20150715*/
    u8 boot_mode = 0;
    char version[60] = {0}; //modify for TW firmware update fail by qinxinjun@yulong.com in 20150708
/*end open TW auto update fw by qinxinjun@yulong.com in 20150715*/
        GTP_INFO("GT915L TW PROBE! \n");
...
//#ifdef VELOCITY_CUSTOM
    ...
    touchscreen_set_ops(&goodix_ts_ops);
...
static void gtp_esd_check_func(struct work_struct *work)
{
    s32 i = 0;
    s32 ret = -1;
    u8 esd_buf[3] = {0x00};//modify for esd check by qinxinjun@yulong.com in 20150724
...
/*start to modify for esd check by qinxinjun@yulong.com in 20150724*/
            esd_buf[0] = 0x01;
            esd_buf[1] = 0x01;
            esd_buf[2] = 0x01;
            i2c_write_bytes(i2c_client_point, 0x4226,esd_buf,3);
/*end to modify for esd check by qinxinjun@yulong.com in 20150724*/
            msleep(50);
            force_reset_guitar();
        }
    }
...
#ifdef TPD_SMART_HULL
/***********************************************************************************************************************
 *Functionality: check the smart case status
 *Parameters: x is current touch point X coordinate, y is current touch point Y coordinate
 *Return Value: integer return value
 *                    0 is continue to report touch point
 *                    -1 is break to report touch point and make the touch event function to return normally
 *Creator: sunxuebin at 2014-08-26 for smart case functionality
 ***********************************************************************************************************************/
static int tpd_smart_hull_check(int x, int y)
{
    if (MODE_WINDOW == tp_mode)//smart hull is close
    {
        //read tpd touch status
        if( (x >= 30 && x <= 690) && (y >= 30 && y <= 790) )//modify to report touch point within the range by qinxinjun@yulong.com in 20150629
        {
            return 0; //continue to report touch point
        }
        else//if touch point out of the range
        {
            return -1;//break to report touch point
        }
    }
    else
    {
        return 1; //continue to report touch point
    }
}
#endif
...
#ifdef TPD_SMART_HULL 
        u8 tpd_in_range_flag = 0;
        u8 tpd_out_range_flag = 0;
#endif
...
#if GTP_GESTURE_WAKEUP
//modified by sunxuebin start for gesture wake up
...
/*start to modify for deleting laft and right slide to screen off by qinxinjun@yulong.com in 20150607*/
/*                                        input_report_key(tpd->dev, KEY_POWER, 1);*/
/*                                        input_sync(tpd->dev);*/
/*                                        input_report_key(tpd->dev, KEY_POWER, 0);*/
/*                                        input_sync(tpd->dev);*/
/*end to modify for deleting laft and right slide to screen off by qinxinjun@yulong.com in 20150607*/
...
//modified by sunxuebin end for gesture wake up
#endif
...
#ifdef TPD_SMART_HULL  
                                ret = tpd_smart_hull_check(input_x, input_y);
                                if(0 == ret) //current mode is smart hull mode and x,y within the range
                                {
                                        tpd_in_range_flag = 1;
                                }
                                else if (-1 == ret)  //current mode is smart hull mode and x,y is out of the range 
                                {
                                        tpd_out_range_flag = 1;
                                        continue;
                                }
                  
                                if ((1==tpd_out_range_flag) && (0 == tpd_in_range_flag) && (1 == pre_tpd_up_flag))
                                {
                                        pre_tpd_up_flag = tpd_in_range_flag;
...
static s8 gtp_enter_doze(struct i2c_client *client)
{
    ...
    GTP_DEBUG_FUNC();
#if GTP_DBL_CLK_WAKEUP
    if(support_gesture & TW_SUPPORT_DOUBLE_CLICK_WAKEUP)  //add by sunxuebin for gesture wake up
        i2c_control_buf[2] = 0x09;
#endif
    sprintf(wakeup_string_buffer,"none");
...
#if GTP_GESTURE_WAKEUP
//modified by sunxuebin for gesture wake up
    //sprintf(wakeup_string_buffer,"none");//modify for gesture wakeup function by qinxinjun@yulong in 20150516
    if(support_gesture & TW_SUPPORT_GESTURE_IN_ALL) 
    {
                GTP_INFO("[sxb] support_gesture is %x\n", support_gesture);
        ret = gtp_enter_doze(i2c_client_point);
    }
    else
#endif
    {
        printk("[sxb] %s func debug here!\n", __func__);
        mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
