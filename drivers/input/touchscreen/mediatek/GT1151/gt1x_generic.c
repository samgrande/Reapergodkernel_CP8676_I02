...
s32 gt1x_i2c_read_dbl_check(u16 addr, u8 * buffer, s32 len)
{
    ...
    if (len > 16) {
        GTP_ERROR("i2c_read_dbl_check length %d is too long, do not exceed %d", len, (int)sizeof(buf));
        return ERROR;
    }
...
s32 gt1x_init_panel(void)
{
    ...
    GTP_INFO("lsl:RESOLUTION: %d * %d, trigger: %d", GTP_MAX_WIDTH, GTP_MAX_HEIGHT, GTP_INT_TRIGGER);
    ...
    GTP_INFO("lsl: X_MAX=%d,Y_MAX=%d,TRIGGER=0x%02x,WAKEUP_LEVEL=%d", gt1x_abs_x_max, gt1x_abs_y_max, gt1x_int_type, gt1x_wakeup_level);
...
void gt1x_power_reset(void)
{
    ...
    for (i = 0; i < 5; i++) {
        if (gt1x_reset_guitar() < 0)
            continue;
        if (gt1x_send_cfg(gt1x_config, gt1x_cfg_length) < 0) {
            msleep(500);
            continue;
        }
        break;
